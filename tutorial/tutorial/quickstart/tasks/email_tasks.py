...
@shared_task(bind=True, max_retries=3, default_retry_delay=60)
def send_blog_notification_email_smtp(self, current_user_username, blog_title, subscribers_emails):
    """
    Alternative method using direct SMTP (if needed)
    """
    try:
        if not subscribers_emails:
            return {"status": "warning", "message": "No subscribers to notify"}
        ...
        try:
            ...
            logger.error(f"Max retries exceeded for SMTP email task: {str(exc)}")
            return {"status": "failed", "error": str(exc)}
